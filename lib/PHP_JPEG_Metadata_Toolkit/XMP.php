...
/**
 * Generates an array with information contained in an Extensible Metadata
 * Platform (XMP) tree structure array,a s retrieved with
 * {@see read_XMP_array_from_text}.
 *
 * @param array $XML_array
 * @return array
 */
function Interpret_XMP_to_Array(array $XMP_array)
{
	$output = array();
                        ...
                        if ( ( $RDF_Item['tag'] == "rdf:Description" ) && ( array_key_exists( 'children', $RDF_Item ) ) )
                        {
                                ...
                                if ( array_key_exists( 'children', $RDF_Item ) )
                                {
                                        ...
                                        foreach( $RDF_Item['children'] as $child_item )
                                        {
                                                ...
                                                list($tag_caption, $value_str, $tag_key) = Interpret_RDF_Item( $child_item );

                                                // Escape the text of the caption for html
                                                $tag_caption = HTML_UTF8_Escape( $tag_caption );
                                                // Escape the text of the value for html
                                                $value_str = HTML_UTF8_Escape( $value_str );

												$removeDescriptions = array('List of Alternates:', 'Unordered List:', 'Ordered List:');
												$value_str_array = explode("\n", $value_str);
												if (in_array($value_str_array[0], $removeDescriptions)) {
													array_shift($value_str_array);
												}
												$value_str = implode("\n", $value_str_array);

												$output[$key][$tag_key] = $value_str;
                                        }
                                }
...
function Interpret_RDF_Item( $Item )
{
        ...
        return array($tag_caption, $value_str, $Item['tag']);
}
